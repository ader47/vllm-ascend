...
class TestNPUModelRunnerKVCache(unittest.TestCase):
    def _build_runner(self):
        ...
        # Merged _allocate_kv_cache_tensors reads parallel_config + num_layers
        # for the layerwise-reuse guard (a no-op here, but it must not crash).
        runner.parallel_config = MagicMock()
        runner.model_config.get_num_layers.return_value = 1
        backend = MagicMock()
