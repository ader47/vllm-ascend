    ...
    at::Tensor copy_counts)
{
    ...
    TORCH_CHECK(query.dim() == 3 && query.size(0) > 0 &&
                ...
                "DSA A5 MTP LIM query must be float8_e4m3fn "
                "[T,32|64,128].");
    const int64_t total_query_rows = query.size(0);
    const int64_t heads = query.size(1);
    TORCH_CHECK(index_weights.dim() == 2 &&
                    index_weights.size(0) == total_query_rows &&
                    index_weights.size(1) == heads &&
                    index_weights.scalar_type() == at::kBFloat16 &&
                    index_weights.stride(1) == 1 &&
                    index_weights.stride(0) >= heads,
                "DSA A5 MTP LIM weights must be BF16 [T,N_IDX] and may "
                "only be strided on the query-row axis.");
    TORCH_CHECK(query_dequant_scale.dim() == 2 &&
                    query_dequant_scale.size(0) == total_query_rows &&
                    query_dequant_scale.size(1) == heads &&
                    query_dequant_scale.scalar_type() == at::kFloat,
                "DSA A5 MTP LIM query scales must be FP32 [T,N_IDX].");
    TORCH_CHECK(index_key_cache.dim() == 4 &&
                ...
                "DSA A5 MTP LIM key cache must be float8_e4m3fn "
                "[blocks,128,1,128].");
    TORCH_CHECK(index_key_dequant_scale.dim() == 3 &&
                ...
                "DSA A5 MTP LIM key scales must be FP32 [blocks,128,1].");
    TORCH_CHECK(index_key_cache.stride(1) == head_dim &&
                ...
                "DSA A5 MTP LIM permits padding only on the Indexer "
                "cache block axis.");
    ...
    for (const at::Tensor* tensor : int_tensors) {
        TORCH_CHECK(tensor->scalar_type() == at::kInt,
                    "DSA A5 MTP LIM metadata and outputs must be int32.");
    }
    TORCH_CHECK(actual_seq_lengths_query.dim() == 1 &&
                    actual_seq_lengths_query.size(0) > 0,
                "DSA A5 MTP LIM actual_seq_lengths_query must be [B].");
    const int64_t batch = actual_seq_lengths_query.size(0);
    TORCH_CHECK(total_query_rows >= batch &&
                    total_query_rows <= batch * max_queries_per_request,
                "DSA A5 MTP LIM requires 1..4 query rows per request.");
    TORCH_CHECK(index_block_table.dim() == 2 &&
                ...
                "DSA A5 MTP LIM request metadata shapes are invalid.");
    ...
    for (const at::Tensor* tensor : tensors) {
        TORCH_CHECK(tensor->device() == device,
                    "all DSA A5 MTP LIM tensors must share one device.");
    }
