    ...
    gert::TilingContext *context)
{
    ...
    const int64_t totalQueryRows = query.GetDim(0);
    const int64_t heads = query.GetDim(1);
    const int64_t batch = queryLengths.GetDim(0);
    if (batch <= 0 || totalQueryRows < batch ||
        totalQueryRows > batch * MAX_QUERIES_PER_REQUEST ||
        (heads != 32 && heads != 64) ||
        weights.GetDim(0) != totalQueryRows ||
        weights.GetDim(1) != heads ||
        queryScale.GetDim(0) != totalQueryRows ||
        queryScale.GetDim(1) != heads ||
        blockTable.GetDim(0) != batch || candidate.GetDim(0) != batch ||
        finalLengths.GetDim(0) != batch || modes.GetDim(0) != batch ||
        entries.GetDim(0) != batch || key.GetDim(0) <= 0 ||
        keyScale.GetDim(0) != key.GetDim(0) || pool.GetDim(0) <= 0 ||
        pool.GetDim(1) < 2 ||
    ...
    if (weightStride == nullptr || keyStride == nullptr ||
        scaleStride == nullptr || *weightStride < heads ||
