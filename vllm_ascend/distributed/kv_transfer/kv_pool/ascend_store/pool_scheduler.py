...
class KVPoolScheduler:
    ...
    def get_num_new_matched_tokens(
        ...
        query_start_block = 0 if self.layerwise_offload else min(num_computed_tokens // self._block_size, num_blocks)
        ...
        if self.use_layerwise:
            ...
            exists_states = self.store_scheduler.batch_is_exist(keys_to_query)
        ...
        force_layerwise_load = self.layerwise_offload and num_external_hit_tokens > 0
    ...
    def update_state_after_alloc(self, request: "Request", blocks: "KVCacheBlocks", num_external_tokens: int):
        ...
        if num_external_tokens == 0:
            ...
            self.load_specs[request.request_id].can_load = (
                self.layerwise_offload and self.load_specs[request.request_id].kvpool_cached_tokens > 0
            )
            return
            ...
            for i, req_id in enumerate(cached_reqs.req_ids):
                ...
                # decode/chunked request
                    ...
                    if request_tracker is None:
                        raise ValueError(
                            f"Request {req_id} is not in _request_trackers, but it is scheduled to be cached"
                        )
