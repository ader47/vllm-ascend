            ...
            for idx in range(len(kv_cache_tensor.shared_by)):
                ...
                if (
                    ...
                    if self.use_sparse and not self.use_offload:
                        # Non-offload sparse: pr-11647 moves the indexer into a
                        # separate AscendSFAIndexerCacheSpec, so the MLA cache is a
                        # plain packed-C8 or k/v tensor with no packed indexer leg.
                    ...
                    # Compute final tensor sizes for the offload / non-sparse path
                    # (the non-offload sparse branch set k/v sizes above). No packed
                    # indexer leg here: under pr-11647 the indexer is allocated as a
                    # separate AscendSFAIndexerCacheSpec.
                    scale_tensor_size = None
                    if not (self.use_sparse and not self.use_offload):
                        k_tensor_size = int(kv_cache_tensor.size // k_tensor_split_factor)
