...
def is_direct_sfa_host_offload(vllm_config: VllmConfig) -> bool:
    """Whether this worker is the PD decode host-cache consumer.

    ``use_offload`` is intentionally meaningful only on the D node. The P
    producer and AscendStore paths keep their normal paged cache layouts.
    """
    kv_transfer_config = getattr(vllm_config, "kv_transfer_config", None)
    additional_config = getattr(vllm_config, "additional_config", None) or {}
    return (
        kv_transfer_config is not None
        and kv_transfer_config.kv_connector == "SFAPDCpuOffloadConnector"
