...
四列状态共用一个 ``CpuGpuBuffer``：

``stage, target_budget, sparse_budget, resident_valid_tokens``

CPU view 服务 host 控制面，固定地址 device view 预留给后续 eager/graph
共同消费。P4 只建立投影并修正 ENTER 的 resident block table，不主动触发
H2D；P5/P6 接算子时应复用这个 owner，而不是另建 graph-only metadata。
"""
...
class DSAInputBatchCacheLayout:
        ...
        *,
        ...
        for source_row, request_id in enumerate(projection.request_ids):
            ...
            if row is None or row >= row_count:
                raise RuntimeError(
                    f"DSA cache-layout projection has no matching InputBatch row: request_id={request_id!r}, row={row}"
                )
    ...
    def copy_to_device(self, row_count: int | None = None) -> torch.Tensor:
        ...
        if self.row_count < self.max_num_reqs:
            padding = slice(self.row_count, self.max_num_reqs)
            self.stages_cpu[padding] = _INVALID_STAGE
            self.target_resident_budget_tokens_cpu[padding] = 0
            self.sparse_budget_tokens_cpu[padding] = 0
            self.resident_valid_tokens_cpu[padding] = _INVALID_RESIDENT_LENGTH
        # 四列采用 SoA 布局，每一列都是可直接传给自定义算子的连续向量。
        # buffer 只有 4 * max_num_reqs 个 int32，整块复制既保持一次 H2D，
