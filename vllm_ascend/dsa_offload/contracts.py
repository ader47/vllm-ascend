...
# budget 按普通 Q=1 / MTP target Q=4 各自的算子合同取值。该 resident
# budget 只属于 target DSA 数据面，不描述 MTP proposer 的 full cache。
DSA_LIDU_OUTPUT_CAPACITY = 16384
DSA_LIDU_TOKEN_CAPACITY = 1 << 18
DSA_LIDU_CACHE_ROW_ALIGNMENT = 256
DSA_NOMTP_SUPPORTED_RESIDENT_BUDGETS = (6144, 10240, 12288)
DSA_MTP_SUPPORTED_RESIDENT_BUDGETS = (8192, 10240, 12288)
DSA_LIDU_SUPPORTED_RESIDENT_BUDGETS = tuple(
    sorted(
        set(DSA_NOMTP_SUPPORTED_RESIDENT_BUDGETS)
        | set(DSA_MTP_SUPPORTED_RESIDENT_BUDGETS)
    )
)
DSA_MTP_MAX_SPECULATIVE_TOKENS = 3
...
# LIM 输出、QSFA 输入的固定列宽：非 MTP 一块 tail，MTP 两块 parity tail。
DSA_A5_NOMTP_ATTENTION_CAPACITY = DSA_SFA_COMPUTE_TOPK + DSA_REQUIRED_CACHE_BLOCK_SIZE
DSA_A5_MTP_ATTENTION_CAPACITY = DSA_SFA_COMPUTE_TOPK + 2 * DSA_REQUIRED_CACHE_BLOCK_SIZE
